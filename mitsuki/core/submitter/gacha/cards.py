...
class CardSubmitter(BaseSubmitter):
  ...
  async def from_rosc2y_yaml(cls, data: dict[str, dict[str, Any]]):
    """
    Create a gacha card submitter from a yaml generated by rosc2y, a csv-to-yaml tool for Mitsuki Gacha roster updates.

    The data follows the format {<id>: {<field>: <value>, ...}, ...}, where the object inside each id corresponds to
    data of each card. The following fields (besides the card id) are available:
    - name: str (required)
    - rarity: int (required)
    - type: str (required)
    - series: str (required)
