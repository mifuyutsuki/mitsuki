...
class GachaGalleryView(GalleryPaginatorMixin, View):
    ...
    return [
      {
        "card_id": card.id,
        "card_name": card.name,
        "card_name_esc": escape_text(card.name),
        "card_type": card.type,
        "card_type_esc": escape_text(card.type),
        "card_series": card.series,
        "card_series_esc": escape_text(card.series),
        "card_blurb": (
          (("★" * card.rarity) if card.rarity <= 6 else f"{card.rarity}★") + f" • {card.type} • {card.series}"
        ),
        "card_image": card.image,
        "card_rarity_color": self.card_cache.rarities[card.rarity].color,
        ...
        "card_last_rolled_blurb": "Last rolled: {}".format(
          card.last_rolled.format("f")
        ) if card.last_rolled != card.first_rolled else "\n"
      }
