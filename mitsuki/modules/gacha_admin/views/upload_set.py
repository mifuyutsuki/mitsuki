...
from mitsuki.core.submitter import CardPackSetSubmitter
from mitsuki.modules.gacha_admin import customids


@attrs.define(slots=False)
class SetUploadPromptView(View):
  submitter: CardPackSetSubmitter
  ...
  def components(self):
    return [
      ipy.ContainerComponent(
        ipy.TextDisplayComponent("## Upload Card Pack Set"),
        ipy.SeparatorComponent(divider=True),
        ipy.TextDisplayComponent(
          "Current card pack set count: **${before_count}**\n"
          "After this operation, the set count will be **${after_count}**.\n"
          "* Sets to add: **${add_count}**\n"
          "* Sets to edit: **${edit_count}**\n"
          "* Sets to remove: **${remove_count}**\n"
      ...
      ipy.ActionRow(
        ipy.Button(
          ...
          custom_id=customids.SET_UPLOAD.id(self.submitter.id),
        ),
      )
    ]
...
class SetUploadDoneView(View):
  ...
  def components(self):
    return [
      ipy.ContainerComponent(
        ipy.TextDisplayComponent("## Uploaded Card Pack Set"),
        ipy.SeparatorComponent(divider=True),
        ipy.TextDisplayComponent(
          "* Added sets: **${add_count}**\n"
          "* Edited sets: **${edit_count}**"
        ),
        ipy.SeparatorComponent(divider=True),
        ipy.TextDisplayComponent(
          "-# {} [Admin]: /gacha-admin upload".format(self.caller.tag)
