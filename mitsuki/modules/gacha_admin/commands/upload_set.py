...
class SetUpload(libcmd.ReaderCommand):
  ...
  async def run(self, file: ipy.Attachment):
    ...
    try:
      ...
      raise errors.BadFile(expect="Card Pack Set YAML file")

    submitter = await CardPackSetSubmitter.from_data(data)
    await views.SetUploadPromptView(self.ctx, submitter).send(ephemeral=True)
  ...
  async def proceed(self, id: str):
    ...
    submitter = await CardPackSetSubmitter.fetch(id)
    if not submitter:
      raise errors.ObjectNotFound("Pack Set Update")

    async with begin_session() as session:
      await submitter.execute(session)
      await views.SetUploadDoneView(self.ctx, submitter).send(ephemeral=True)
