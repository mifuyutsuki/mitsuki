...
from mitsuki.core.submitter import CardPackSubmitter, CardPackSubmitterEntry
from mitsuki.core.gacha import CardCache
from mitsuki.modules.gacha_admin import views


class PackUpload(libcmd.ReaderCommand):
  ...
  async def proceed(self, id: str):
    ...
    submitter = await CardPackSubmitter.fetch(id)
    if not submitter:
      raise errors.ObjectNotFound("Pack Update")

    async with begin_session() as session:
      await submitter.execute(session)
      await views.PackUploadDoneView(self.ctx, submitter).send(ephemeral=True)
