...
class SeasonUpload(libcmd.ReaderCommand):
  ...
  async def run(self, file: ipy.Attachment):
    ...
    submitter = await GachaSeasonSubmitter.from_data(data)
    await views.SeasonUploadPromptView(self.ctx, submitter).send(ephemeral=True)
  ...
  async def proceed(self, id: str):
    ...
    submitter = await GachaSeasonSubmitter.fetch(id)
    if not submitter:
      raise errors.ObjectNotFound("Season Update")

    async with begin_session() as session:
      await submitter.execute(session, overwrite=True)
      await views.SeasonUploadDoneView(self.ctx, submitter).send(ephemeral=True)
